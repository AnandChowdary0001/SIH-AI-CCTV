...
# Names in this file receive a high-priority red watchlist alert.
# Keep this list manually verified for your project/demo.
WATCHLIST_ALERT_LABEL = "ISIS TERRORIST ALERT"

YOLO_MODEL = "yolo11n.pt"

print("\n========================================")
print("       SIH CCTV CAMERA SELECT")
print("========================================")
...
# Check the real watchlist folder every N seconds.
...
def load_watchlist():
    ...
    for filename in sorted(os.listdir(WATCHLIST_FOLDER)):
        if not filename.lower().endswith(valid_extensions):
            continue

        person_name = get_name_from_watchlist_filename(filename)

        if person_name:
            watchlist_names.add(person_name.lower())

    print("\nWatchlist loaded:", len(watchlist_names), "person(s)")

    for name in sorted(watchlist_names):
...
while True:
    ...
    if (
        ...
        previous_watchlist = set(watchlist_names)

        load_watchlist()

        if watchlist_names != previous_watchlist:

            added = sorted(
                watchlist_names - previous_watchlist
            )

            removed = sorted(
                previous_watchlist - watchlist_names
            )
    ...
    for face in faces:
        ...
        # ====================================================
        # TARGET / WATCHLIST DETECTION
        # ====================================================

        # ONLY an identity explicitly present in the current
        # data/watchlist folder is a target/watchlist alert.
        # Reference database membership alone NEVER creates an alert.
        if (
            display_name != "UNKNOWN"
            and
            display_name != ""
            and
            is_watchlist_person(display_name)
        ):
            watchlist_detected = True
            watchlist_name = display_name
            watchlist_track_id = best_track_id
        ...
        # Red = manually configured watchlist person.
        # Blue = ordinary recognized/unknown face.
        if is_watchlist_person(display_name):
            box_color = (0, 0, 255)
            box_thickness = 3
        else:
            box_color = (255, 0, 0)
        ...
        label_color = (0, 0, 255) if is_watchlist_person(display_name) else (255, 0, 0)
    ...
    if watchlist_detected:
        ...
        if watchlist_track_id is not None:
            alert_text = (
                f"ID {watchlist_track_id} | {watchlist_name}"
            )
        else:
            alert_text = f"{watchlist_name}"
        ...
        # Normal recognized reference person.
        cv2.rectangle(
            frame,
            (15, 80),
            (430, 135),
